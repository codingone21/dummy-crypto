...
1. Input numbers are integers, as shown in the example. The application uses Long as the integer format. For the actual calculations, the numbers are transformed to BigDecimals.
...
5. There is only single key used for encryption/decryption, so this is generated & statically used within the CryptoUtils class. The key is not revealed to the API user.
